...
namespace WPAppStudio.BackgroundProcess.Agents
{
    public class CycleTileAgent
    {
        ...
        public void CreateCycleData()
        {
            ...
            //Remove not used images in LiveTileFolder
            using (var isoStore = IsolatedStorageFile.GetUserStoreForApplication())
            {
                var notUsedImages = isoStore.GetFileNames(LiveTileFolder).Skip(9);
                foreach(var notUsedImage in notUsedImages)
                    isoStore.DeleteFile(notUsedImage);
            }
        }
    }
}